...
func RTPDepay(codec *core.Codec, handler core.HandlerFunc) core.HandlerFunc {
	...
	return func(packet *rtp.Packet) {
		...
		// https://github.com/hamza-farouk/go2rtc/issues/675
		...
		if packet.Marker && len(payload) < PSMaxSize {
			...
			case NALUTypeSPS, NALUTypePPS:
				...
				return
			case NALUTypeSEI:
				// RtspServer https://github.com/hamza-farouk/go2rtc/issues/244
		...
		if NALUType(payload) == NALUTypeSPS && binary.BigEndian.Uint32(payload) >= PSMaxSize {
			...
			// https://github.com/hamza-farouk/WebRTC/issues/391
			// https://github.com/hamza-farouk/WebRTC/issues/392
